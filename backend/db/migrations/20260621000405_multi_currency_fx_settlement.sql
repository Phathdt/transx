...
ALTER TABLE transfers
    ADD COLUMN source_amount NUMERIC(20, 4) CHECK (source_amount IS NULL
        OR source_amount > 0),
        ADD COLUMN source_currency TEXT NOT NULL DEFAULT '',
        ADD COLUMN destination_amount NUMERIC(20, 4) CHECK (destination_amount IS NULL
        OR destination_amount > 0),
        ADD COLUMN destination_currency TEXT NOT NULL DEFAULT '',
        ADD COLUMN source_fx_rate NUMERIC(20, 12) CHECK (source_fx_rate IS NULL
        OR source_fx_rate > 0),
        ADD COLUMN destination_fx_rate NUMERIC(20, 12) CHECK (destination_fx_rate IS NULL
        OR destination_fx_rate > 0),
