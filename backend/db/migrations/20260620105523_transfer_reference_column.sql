...
ALTER TABLE transfers ADD COLUMN reference TEXT;
...
DECLARE
    ...
    n BIGINT;
BEGIN
    FOR r IN SELECT id, transfer_type, created_at FROM transfers WHERE reference IS NULL LOOP
        body := '';
        ...
        END LOOP;
        UPDATE transfers
        SET reference = (CASE WHEN r.transfer_type = 'EXTERNAL' THEN 'ETN-' ELSE 'ITN-' END) || body
        WHERE id = r.id;
    END LOOP;
END $$;

ALTER TABLE transfers ALTER COLUMN reference SET NOT NULL;
...
ALTER TABLE transfers DROP COLUMN reference;
-- +goose StatementEnd
